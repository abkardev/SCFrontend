...
import { LangProvider, AuthProvider, useLang, useAuth } from './context/AppContext';
import Layout from './components/Layout';
import LoginPage from './components/auth/LoginPage';
import { RegisterPage, ForgotPasswordPage, ResetPasswordPage } from './components/auth/AuthPages';

const DashboardPage   = lazy(() => import('./pages/DashboardPage'));
const BookingsPage    = lazy(() => import('./pages/BookingsPage'));
const DoctorsPage     = lazy(() => import('./pages/DoctorsPage'));
const CalendarPage    = lazy(() => import('./pages/CalendarPage'));
const SlotManagerPage = lazy(() => import('./pages/SlotManagerPage'));
...
// ─── Full-page spinner shown while auth state is being verified ───────────────
function AuthLoader() {
  return (
...
function ThemedApp() {
  ...
  const theme = createTheme({
    ...
    components: {
      MuiCssBaseline: {
        styleOverrides: `
          @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800&family=Cairo:wght@400;500;600;700;800&display=swap');
    ...
    <ThemeProvider theme={theme}>
      ...
      <BrowserRouter>
        <Routes>
          ...
          >
            {/* Root → login if not authed (handled by RequireAuth above), dashboard if authed */}
            ...
            <Route
              path="users"
              element={
                <RequireRole role="superadmin">{S(UsersPage)}</RequireRole>
              }
            />
            ...
            <Route
              path="audit-logs"
              element={
                <RequireRole role="superadmin">{S(AuditLogsPage)}</RequireRole>
              }
            />
