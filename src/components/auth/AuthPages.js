...
import { authRegister, authForgotPassword, authResetPassword } from '../../services/api';
...
export function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Send auth token so backend can verify caller is admin/superadmin
      ...
      const msg = err.response?.data?.message || (isRTL ? 'فشل التسجيل' : 'Registration failed');
      const restricted = err.response?.data?.restricted;
      setError(restricted
        ? (isRTL ? 'التسجيل مقيد للمشرفين فقط. يرجى التواصل مع مشرف النظام لإنشاء حساب.' : 'Registration is restricted to administrators. Please ask your admin to create an account for you.')
        : msg
      );
  ...
  return (
    <AuthShell>
      ...
      <Typography color="text.secondary" mb={3} fontSize={13}>
        {isRTL ? 'إنشاء حساب مشرف — يحتاج موافقة المشرف العام' : 'Create admin account — requires superadmin approval'}
      </Typography>
