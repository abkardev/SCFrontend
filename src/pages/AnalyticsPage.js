...
import { getBookings, getDoctors } from '../services/api';

const STATUS_COLORS  = { pending:'#F59E0B', confirmed:'#0A6EBD', completed:'#10B981', cancelled:'#EF4444', 'no-show':'#8B5CF6' };
...
export default function AnalyticsPage() {
  ...
  const noShowCur    = filtered.filter(b=>b.status==='no-show').length;
  ...
  filtered.forEach(b => {
    ...
    const name = b.doctorId?.name || (isRTL?'غير معروف':'Unknown');
    if (!doctorMap[id]) doctorMap[id] = { name, total:0, completed:0, cancelled:0, noShow:0 };
    doctorMap[id].total++;
    if(b.status==='completed') doctorMap[id].completed++;
    if(b.status==='cancelled') doctorMap[id].cancelled++;
    if(b.status==='no-show') doctorMap[id].noShow++;
  });
            ...
            {doctorPerf.map((d, i) => (
              <Box key={i} sx={{ display:'grid', gridTemplateColumns:'1fr 80px 80px 80px 80px 100px', gap:1, px:2, py:1.5, borderRadius:2, '&:hover':{bgcolor:'#F8FAFC'}, transition:'all 0.15s', alignItems:'center' }}>
                <Box sx={{ display:'flex', alignItems:'center', gap:1.5 }}>
                  <Avatar sx={{ width:32, height:32, fontSize:13, fontWeight:700, bgcolor:'#EFF6FF', color:'#0A6EBD' }}>{d.name.charAt(0)}</Avatar>
                  <Typography fontWeight={600} fontSize={13}>{isRTL?`د. ${d.name}`:`Dr. ${d.name}`}</Typography>
                </Box>
