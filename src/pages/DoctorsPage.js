...
import { getDoctors, createDoctor, updateDoctor, deleteDoctor, syncFromGoogle } from '../services/api';
              ...
              <Card elevation={0} sx={{
                ...
                <CardActions sx={{ px: 2, py: 1.2, gap: 0.5 }}>
                  ...
                  <Tooltip title={isRTL ? 'مزامنة من جوجل' : 'Sync from Google'}>
                    <IconButton size="small" onClick={() => handleSync(d._id, docName(d))} disabled={syncing === d._id}
                      sx={{ '&:hover': { bgcolor: '#F0FDF4', color: '#10B981' } }}>
                      <SyncRoundedIcon fontSize="small" sx={{
                        animation: syncing === d._id ? 'spin 1s linear infinite' : 'none',
