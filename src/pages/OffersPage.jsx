...
export default function OffersPage() {
  ...
  const handleToggle = async (offer) => {
    try { await updateOffer(offer._id, { isActive: !offer.isActive }); load(); }
    catch { notify('Error','error'); }
  };
    ...
    <Box>
      ...
      <Grid container spacing={2.5}>
        {loading ? Array.from({length:4}).map((_,i) => (
          ...
          <Grid item xs={12} sm={6} lg={4} key={offer._id}>
              ...
              <Box sx={{ p:2.5 }}>
                ...
                <Box sx={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  ...
                  <Box>
                    ...
                    <Tooltip title={t('delete')}><IconButton size="small" color="error" onClick={()=>handleDelete(offer._id)}><DeleteRoundedIcon fontSize="small"/></IconButton></Tooltip>
                  </Box>
                </Box>
              </Box>
            </Paper>
