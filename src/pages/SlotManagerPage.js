...
import { getDoctors, getBlockedSlots, blockSlot, unblockSlot, getBookings } from '../services/api';
...
export default function SlotManagerPage() {
  ...
  const getBlockedId = (time) => blockedList.find((b) => b.time === time)?._id;
  const getWholeDayId = () => blockedList.find((b) => b.isWholeDay)?._id;
      ...
      <Paper elevation={0} sx={{ p: 2.5, mb: 3, display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'flex-end', border: '1px solid rgba(148,163,184,0.15)' }}>
        <FormControl size="small" sx={{ minWidth: 220 }}>
          ...
          <Select value={selectedDoctor} label={t('selectDoctor')} onChange={(e) => setSelectedDoctor(e.target.value)}>
            {doctors.map((d) => <MenuItem key={d._id} value={d._id}>Dr. {d.name}</MenuItem>)}
          </Select>
        </FormControl>
        <TextField
