...
import { getUsers, updateUserStatus, updateUserRole, deleteUser } from '../services/api';
...
export default function UsersPage() {
  ...
  const handleRole = useCallback(async (id, newRole) => {
    const prev = usersRef.current.find(u => u._id === id)?.role;
    setUsers(cur => cur.map(u => u._id === id ? { ...u, role: newRole } : u));
    try {
      await updateUserRole(id, newRole);
      notify('Role updated', 'تم تحديث الدور');
    } catch (err) {
      setUsers(cur => cur.map(u => u._id === id ? { ...u, role: prev } : u));
  ...
  const handleStatus = async (id, status) => {
    try {
      ...
      setUsers(cur => cur.map(u => u._id === id ? { ...u, status } : u));
  ...
  // ── Columns — using handleRole from outer scope (stable ref via useCallback) ──
  const columns = [
    {
      field: 'name',
    ...
    { field: 'email', headerName: isRTL ? 'البريد الإلكتروني' : 'Email', flex: 1, minWidth: 190 },
    {
      ...
      renderCell: (p) => (
        <Select
          ...
          onChange={(e) => {
            e.stopPropagation();
            handleRole(p.row._id, e.target.value);
          }}
        >
          {ROLES.map(r => (
    ...
    },
    {
      ...
      width: 160,
      valueGetter: (p) => p.row.lastLogin
        ? new Date(p.row.lastLogin).toLocaleString(isRTL ? 'ar-SA' : 'en-US')
        : '—',
    },
    {
      field: 'createdAt',
      ...
      width: 130,
      valueGetter: (p) => new Date(p.row.createdAt).toLocaleDateString(isRTL ? 'ar-SA' : 'en-US'),
    },
    {
      field: 'actions',
        ...
        <Box display="flex" gap={0.5}>
          {p.row.status !== 'approved' && (
            <Tooltip title={isRTL ? 'الموافقة على المستخدم' : 'Approve user'}>
              <IconButton size="small" color="success"
                onClick={() => setConfirm({ open: true, userId: p.row._id, action: 'approved', labelEn: 'Approve', labelAr: 'موافقة' })}>
          ...
          {p.row.status !== 'rejected' && p.row.role !== 'superadmin' && (
            <Tooltip title={isRTL ? 'رفض المستخدم' : 'Reject user'}>
              <IconButton size="small" color="error" onClick={() => handleStatus(p.row._id, 'rejected')}>
          ...
          {p.row.status !== 'suspended' && p.row.role !== 'superadmin' && (
            <Tooltip title={isRTL ? 'تعليق الحساب' : 'Suspend account'}>
              <IconButton size="small" color="warning" onClick={() => handleStatus(p.row._id, 'suspended')}>
          ...
          {p.row.status === 'suspended' && p.row.role !== 'superadmin' && (
            <Tooltip title={isRTL ? 'إعادة تفعيل الحساب' : 'Reactivate account'}>
              <IconButton size="small" color="success" onClick={() => handleStatus(p.row._id, 'approved')}>
    ...
    <Box>
      ...
      <Paper elevation={0} sx={{ height: 540, borderRadius: 3, overflow: 'hidden', border: '1px solid rgba(148,163,184,0.15)' }}>
        <DataGrid
          ...
          key={users.map(u => `${u._id}:${u.role}:${u.status}`).join('|')}
          rows={users}
          columns={columns}
          loading={loading}
          pageSize={10}
